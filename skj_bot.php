...
function create_order_session($conn,$senderId,$chatId){
    $check_id = check_order_chat_id($conn,$senderId);
   return $check_id;
}

function save_order_image($conn,$senderId,$photo){
    $sql = "UPDATE order_sessions SET order_photo = '$photo' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);

}

function save_order_branchname($conn,$senderId,$branch_name){
    $sql = "UPDATE order_sessions SET branch_name = '$branch_name' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);
}

function save_order_weight($conn,$senderId,$weight){
    $sql = "UPDATE order_sessions SET weight = '$weight' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);
}


function save_order_size($conn,$senderId,$size){
    $sql = "UPDATE order_sessions SET length = '$size' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);
}

function save_order_remark($conn,$senderId,$remark){
    $sql = "UPDATE order_sessions SET remark = '$remark' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);
}

function save_order_due($conn,$senderId,$due){
    $sql = "UPDATE order_sessions SET due_date = '$due' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);
}
function save_order_product_code($conn,$senderId,$product_code){
    $sql = "UPDATE order_sessions SET product_code = '$product_code' WHERE chat_id = '$senderId'";
    $run =  mysqli_query($conn,$sql);
}
...
function create_invoice($conn,$chatId,$invoice_id,$photo_id,$weigth,$branch_name,$length,$remarks,$due_date,$product_code,$img_location){
    $sql = "INSERT INTO  orders (chat_id,invoice_id,photo_id,weight,branch_name,size,remarks,due_date,status,product_code,img_url) 
    VALUES('$chatId','$invoice_id','$photo_id','$weigth','$branch_name','$length','$remarks','$due_date','pending','$product_code','$img_location')";
    $run = mysqli_query($conn,$sql);
...
if(!empty($message) && strpos($message,"admin") === false){
    ...
    switch ($message) {
        ...
        case '/orderstatus':
            file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= To check the status of your order send the following command. 'check order status:Enter Order ID here' ");
            exit(0);
            break;
            # code...
    ...
    if(strpos($message,"check order status:") !== false){
        $query = $text;
        $str = preg_split('/:/',$query);
        $invoice_id = $str[1];
    ...
    if(strpos($message,"place order") !== false){
        $check_id = create_order_session($conn,$senderId,$chatId);
        if($check_id == 1){
...
if($photo != null ){
    save_order_image($conn,$senderId,$photo);
    file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text=Send the weight needed in this format 'Weight:2gmm' ");
    exit(0);

}
if(strpos($message,"branch name:") !== false){
    ...
    $split_text = preg_split('/:/',$message);
    if($split_text[0] == "branch name"){
        $branch_name = $split_text[1];
        save_order_branchname($conn,$senderId,$branch_name);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text=  Send the lenght/size  needed in this format 'Size:20cm' ");
        exit(0);
    }
    
}

if(strpos($message,"weight:") !== false){
    $check_order_session =  check_order_chat_id($conn,$senderId);
    if($check_order_session == 0){
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Please send 'Place order or place stock order' to start a order session  ");
        exit(0);
    ...
    $split_text = preg_split('/:/',$message);
    if($split_text[0] == "weight"){
        $weight = $split_text[1];
        save_order_weight($conn,$senderId,$weight);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Send a branch name in this format 'Branch name:Example Branch name'");
        exit(0);
    }
...
if(strpos($message,"size:") !== false){
    ...
    $split_text = preg_split('/:/',$message);
    if($split_text[0] == "size"){
        $size = $split_text[1];
        save_order_size($conn,$senderId,$size);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Send a remark in this format 'Remark:your remarks ' or if no remarks send as 'Remark:no remarks'");
        exit(0);
    }
    
}
if(strpos($message,"remark:") !== false){
    $check_order_session =  check_order_chat_id($conn,$senderId);
    if($check_order_session == 0){
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Please send 'Place order or place stock order' to start a order session  ");
        exit(0);
    ...
    $split_text = preg_split('/:/',$message);
    if($split_text[0] == "remark"){
        $remark = $split_text[1];
        save_order_remark($conn,$senderId,$remark);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Send due date of choice in this format 'Due:month/date/year' ");
        exit(0);
    }
...
if(strpos($message,"due:") !== false){
    ...
    if($split_text[0] == "due"){
        $due = $split_text[1];
        save_order_due($conn,$senderId,$due);
        $invoice_id = create_invoice_id($conn);
        save_order_invoice_id($conn,$senderId,$invoice_id);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= you've created a new order with Invoice Id ( ".$invoice_id." ) to place this order send the following command 'order now: ".$invoice_id."'");
        exit(0);
    }
...
if(strpos($message,"order now:") !== false){
    ...
    create_invoice($conn,$chatId,$invoice_id,$photo_id,$weight,$branch_name,$length,$remarks,$due_date,$product_code,$img_location);
    delete_order_session($conn,$senderId);
...
if(strpos($message,'place stock order') !== false){
    ...
    if($check_id == 1){
        delete_order_session($conn,$senderId);
        add_new_order_session($conn,$senderId);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text=Please Send the product code of your choice like this 'product code:B0209' ");
        exit(0);
    }else{
        add_new_order_session($conn,$senderId);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text=Please Send the product code of your choice like this 'product code:B0209' ");
...
if(strpos($message,'product code:') !== false){
    $check_order_session =  check_order_chat_id($conn,$senderId);
    if($check_order_session == 0){
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Please send 'place stock order' to start a order session  ");
        exit(0);
    }
    $split_text = preg_split('/:/',$text);
    $code = str_replace(' ','', $split_text[1]);
    ...
    if($pro_count < 1){
        ...
        save_order_image($conn,$senderId,$photo);
        save_order_weight($conn,$senderId,$weight);
        save_order_product_code($conn,$senderId,$code);
        file_get_contents($api.'/sendMessage?chat_id='.$chatId."&text= Send a branch name in this format 'Branch name:Example Branch name'");
        exit(0);
